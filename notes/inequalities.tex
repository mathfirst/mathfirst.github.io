...
Given $x,y>0, p,q>1$ and $1/p+1/q=1$, the inequality
\[
xy\le \frac{1}{p}x^p+\frac{1}{q}y^q,
\]
where the equality holds if and only if $x^p=y^q$.
\begin{proof}
Let $f(x)=e^x$, then $f''>0$. So $f$ is convex. By the Jensen's inequality,
\begin{gather*}
f(\frac{1}{p}\ln x^p+\frac{1}{q}\ln y^q)\le\frac{1}{p}f(\ln x^p)+\frac{1}{q}f(\ln y^q) \\
\Downarrow\\
e^{\frac{1}{p}\ln x^p+\frac{1}{q}\ln y^q}\le\frac{1}{p}e^{\ln x^p}+\frac{1}{q}e^{\ln y^q}
\Longleftrightarrow e^{\frac{1}{p}\ln x^p}e^{\frac{1}{q}\ln y^q}\le\frac{1}{p}x^p+\frac{1}{q}y^q 
\Longleftrightarrow xy\le\frac{1}{p}x^p+\frac{1}{q}y^q,
\end{gather*}
as desired. The equality follows from the condition for the equality of Jensen's inequality to hold with a strictly convex function. 
\end{proof}
